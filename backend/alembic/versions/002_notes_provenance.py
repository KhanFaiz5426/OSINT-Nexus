...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('observation_id', sa.dialects.postgresql.UUID(), sa.ForeignKey('observations.id', ondelete='CASCADE'), nullable=False),
        sa.Column('investigation_id', sa.dialects.postgresql.UUID(), sa.ForeignKey('investigations.id', ondelete='CASCADE'), nullable=False),
    ...
    op.create_index('idx_entity_provenance_investigation', 'entity_provenance', ['investigation_id'])
    
    # Add unique constraint to prevent duplicate provenance records
    op.create_unique_constraint(
        'uq_entity_provenance',
        'entity_provenance',
    ...
    op.create_index('idx_confidence_overrides_investigation', 'confidence_overrides', ['investigation_id'])
