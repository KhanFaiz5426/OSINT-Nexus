...
def generate_probe_variations(
    username: str,
    max_variations: int = 10,
) -> list[str]:
    """Generate username variations optimized for direct platform probing.

    These are confidence-ranked, high-quality variations only. Unlike
    search query variations, these are designed for direct HTTP probes
    where each variation costs a request.

    Variations are generated by:
    1. Original username (always first)
    2. Common separator variations (for multi-part names)
    3. Joined/parts variations (for multi-part names)
    4. A small number of prefix/suffix variations

    This does NOT generate arbitrary/random usernames from a person's name.

    Args:
        username: The original username to generate variations for.
        max_variations: Maximum number of variations to return (default 10).

    Returns:
        Confidence-ranked list of variations, original first.
    """
    if not username or not username.strip():
        return [username]
    ...
    # Split only on separators (no camelCase merge to avoid full-name echo)
    parts = re.split(r"[._\-\s]+", base)
    parts = [p for p in parts if p]

    if len(parts) >= 2:
        # Multi-part: try different separators
        for sep in ["", ".", "-", "_"]:
            var = sep.join(parts)
            if var not in variations:
                variations.append(var)
        ...
        # Reverse parts (last.first)
        reversed_parts = list(reversed(parts))
        for sep in ["", "."]:
            var = sep.join(reversed_parts)
            if var not in variations:
                variations.append(var)

    # Single prefix/suffix variations (limited)
    if len(parts) < 2:
        for suffix in ["0", "1", "dev"]:
            var = f"{base}{suffix}"
            if var not in variations:
                variations.append(var)
    else:
        # For multi-part, try the most common joined variation with a number
        joined = "".join(parts)
        for suffix in ["0", "1"]:
            var = f"{joined}{suffix}"
            if var not in variations:
    ...
    result = [base] + [v for v in unique if v != base][:max_variations]
    logger.info(
        "Generated %d probe variations for '%s'",
        len(result) - 1,
        username,
    )
    return result
