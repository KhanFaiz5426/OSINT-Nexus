...
## Completed Tasks (as of 2026-09-06)

### Phase 3 — OSINT Collectors (Completed)
- **3.1–3.11**: All 11 collectors implemented (DNS, WHOIS, CT, IP-ASN, GitHub, HTTP, Threat Intel, Reddit, Keybase, HackerNews, GitLab)
- **3.12**: Search collector added (DuckDuckGo HTML API) — NEW
- **3.13**: Collector registry, caching, rate limiting implemented
- **3.14**: Celery tasks for all collectors implemented
- **3.15**: Collector tests written and passing

### Phase 5 — Knowledge Graph & Correlation (Completed)
- **5.1–5.10**: Neo4j graph read/write, correlation engine, graph API implemented
- **5.11**: Search collector integration added to correlator — NEW

### Phase 6 — Investigation Orchestrator & AI Planner (Completed)
- **6.1–6.15**: Orchestrator, AI planner, pivot selection, budget/depth control implemented
- **6.9**: Search collector added to initial actions for DOMAIN, URL, USERNAME, ORGANIZATION targets — NEW

### Phase 8 — Reporting (Completed)
- **8.1–8.10**: HTML/PDF/JSON/CSV report generation implemented

### Phase 9 — Testing, Polish & Documentation (Partial)
- **9.1–9.4**: Unit tests, integration tests, collector tests, API contract tests — 500 tests passing
- **9.5**: Frontend component tests — 29 tests passing
- **9.6**: E2E test setup with Playwright — NEW (test file created, needs runtime verification)
- **9.9**: README.md documentation — exists
- **9.10**: OpenAPI spec auto-generated by FastAPI

### New Features Added (This Session)
1. **Search OSINT Collector** (`backend/app/collectors/search_collector.py`)
   - Uses DuckDuckGo HTML API (no API key required)
   - Supports DOMAIN, IP, URL, USERNAME, EMAIL, ORGANIZATION targets
   - Extracts entities from search result snippets
   - Tests: `tests/test_search_collector.py` (16 tests)

2. **AI Validation Tests** (`backend/tests/test_ai_validation.py`)
   - Malformed AI output handling (planner and analyzer)
   - Unsafe pivot detection (private IPs, XSS, SQL injection)
   - Budget/depth edge cases
   - Deterministic fallback verification
   - Tests: 36 tests

3. **Settings Panel** (Frontend)
   - `frontend/src/components/SettingsPanel.tsx`
   - Backend endpoint: `GET /api/v1/config/status`
   - Read-only status display (no secrets exposed)

4. **Playwright E2E Test Setup**
   - `frontend/playwright.config.ts`
   - `frontend/e2e/critical-journey.spec.ts`
...
### High Priority
- **9.6**: Run Playwright E2E tests with actual browser (requires Playwright browsers installed)
- **9.12**: Set up GitHub Actions CI pipeline

### Medium Priority
- **7.1–7.15**: Frontend dashboard improvements (if needed)
- **9.7**: Performance testing with 50+ entities
- **9.8**: Create seed data for sample investigations

### Low Priority
- **9.11**: Security audit (verify no secrets, CORS config, input validation)
- **9.10**: Enhance API documentation
